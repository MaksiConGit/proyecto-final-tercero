<x-template-layout>
    ...
    <div class="container">
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light"><a href="{{ route('exams.index') }}">Examenes /</a></span> Calificaciones
        </h4>
        <x-form-horizontal-icon>
            <x-slot name="titulo">Cargar calificaciones</x-slot>
            <x-slot name="action">{{ route('grades.store') }}</x-slot>
            <x-slot name="method"></x-slot>
            <x-slot name="inputs">

                @livewire('CreateGrades')


            </x-slot>
            <x-slot name="modal">
                <x-modal_template>
                    <x-slot name="titulo">¿Estás seguro que quiere cargar estas calificaciones?</x-slot>
                    <x-slot name="contenido"></x-slot>
                </x-modal_template>
            </x-slot>
            <x-slot name="volver_url">{{ route('grades.index') }}</x-slot>
        </x-form-horizontal-icon>
        @if ($errors->any())
            <ul>
